...
(def ^:const vector-types '(ByteVector ShortVector IntVector))
...
(defn create-species-enum [class-name size]
  (symbol (str "jdk.incubator.vector." (name class-name)) (str "SPECIES_" size)))

(defn create-species-fn [class-name fn-name]
  (symbol (str "jdk.incubator.vector." (name class-name)) (name fn-name)))
